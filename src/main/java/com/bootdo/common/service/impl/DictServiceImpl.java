...
public class DictServiceImpl implements DictService {
    ...
    @Override
    public HashMap<String,Object> getAllDict() {
        HashMap<String,Object> dictList = new HashMap<>();
        List<DictDO>  listType = this.listType();
        List<DictDO>  allDict =dictDao.list(new HashMap<>());
        for(DictDO dictDO :listType){
            List<DictDO> thetype = new ArrayList<>();
                   for(DictDO dto : allDict){
                       if(dictDO.getType().equals(dto.getType()) ){
                           thetype.add(dto);
                       }

                   }
            dictList.put(dictDO.getType(),thetype);
        }
        return dictList;
    }
}
